...
$settings = array(
    'oauth_access_token' => "763391247226662912-lXnob7SOxrlJW9v9JhWOnHJZFzS3JVx",
    'oauth_access_token_secret' => "8Spac3PDeC2sDXwcKOdMtblRE8mqtPHhFIeT99XExPAoF",
    'consumer_key' => "u1nwTgFoebd7IPqntDkhfdnI1",
    'consumer_secret' => "SYGlxQaod0P4RLMHxpUde5WjIwlvweXas90VN2ZGQcIFOBiBis"
);
...
foreach (json_decode($statuses) as $s) {
  ...
  $array["type"] = "twitter";
  if (isset($s->entities->media[0])) {
    $array["picture"] = $s->entities->media[0]->media_url;  
  } else {
    $array["picture"] = "false";
  }
...
foreach ( $feedarray->data as $feed_data )
{
  ...
  if (isset($feed_data->message)) {
    ...
    if (isset($feed_data->attachments->data[0]->title)) {
      $tempArray["message"] = $feed_data->attachments->data[0]->title;
    }
  }
...
foreach (array_merge($array, $finArr) as $a) {
  $comparison[] = $a["time_raw"];
}

$final = array_merge($array, $finArr);
array_multisort($comparison, SORT_DESC, $final);
